...
contract FantomonTrainerGraphicsV2 is Ownable {
    ...
    string public INSTRUCTIONS = "Call tokenURI(tokenId). The returned metadata returns two relevant fields: 'image' and 'overlay'. In HTML, display them in that order ('art' then 'image') via two 'img' elements each of which uses `position: absolute`. This will overlay the Trainer stats SVG on top of the IPFS art PNG. For iOS use 'iosOverlay'";
