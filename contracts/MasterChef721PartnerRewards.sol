...
contract MasterChef721PartnerRewards is Ownable {
    ...
    function updatePool(uint256 _pid) public returns (IERC721, uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        if (block.timestamp <= pool.lastRewardTime) return (pool.lpToken, pool.accRewardsPerShare / 1e12);
    ...
    function claimRewards(uint256 _pid, uint256[] memory _tokenIds) external {
    ...
    function deposit(uint256 _pid, uint256[] memory _tokenIds) external {
        PoolInfo storage pool = poolInfo[_pid];
        // Relevant pool info must be pulled into memory AFTER the pool updates
        (IERC721 lpToken, uint256 accRewardsPerShare) = updatePool(_pid);
    ...
    function withdraw(uint256 _pid, uint256[] memory _tokenIds) external {
        // Relevant pool info must be pulled into memory AFTER the pool updates
        (IERC721 lpToken, uint256 accRewardsPerShare) = updatePool(_pid);
