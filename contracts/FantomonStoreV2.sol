 ...
 * info are here.
 **************************************************************************/
contract FantomonStoreV2 is IFantomonStore, Ownable {
    ...
    function setAttributes(IFantomonAttributes _attrs) external onlyOwner {
        attrs_ = _attrs;
    }
    ...
    // Migrate an Fmon's storage from an old FantomonStore to this one
    function migrateBatch(uint256 _startTok, uint256 _batchSize) external onlyOwner {
        ...
        for (uint256 tok = _startTok; tok < end; tok++) {
            ...
            fmonsInternal_[tok].lvl       = fmon.lvl;
            fmonsInternal_[tok].morph     = fmon.morph;
            fmonsInternal_[tok].xp        = fmon.xp;
            fmonsInternal_[tok].dmg       = fmon.dmg;
            fmonsInternal_[tok].nutrients = fmon.nutrients;
