...
contract FantomonGraphics is IFantomonGraphics, Ownable {
    ...
    function getName(uint256 _tokenId) public view returns (string memory) {
        IFantomonStore fantomon = registry_.fstore_();
        FantomonLib.Attributes memory attrs = fantomon.fmon(_tokenId).attrs;
        if (bytes(fantomon.names_(_tokenId)).length == 0) {
            return string(abi.encodePacked("Unknown ", SPECIES[attrs.species]));
        } else {
            return fantomon.names_(_tokenId);
        }
    }
